export class Monkey {
    ...
    inspectItems() {
       this.startingItems = this.startingItems
            .map(num => Math.floor(this.operation(num) / 3))
        this.itemsInspected += this.startingItems.length
    }
...
export const parseInput = (input: string) => {
    return input.split('\n\n')
        .map(input => {
            ...
            return new Monkey(monkeyId, [], () => 0, () => 0)
        })
}
...
export const generateTestMonkeys = () => ({
    0: new Monkey(0, [79, 98], (old) => old * 19, (newItem) => newItem % 23 === 0 ? 2 : 3),
    1: new Monkey(1, [54, 65, 75, 74], (old) => old + 6, (newItem) => newItem % 19 === 0 ? 2 : 0),
    2: new Monkey(2, [79, 60, 97], (old) => old * old, (newItem) => newItem % 13 === 0 ? 1 : 3),
    3: new Monkey(3, [74], (old) => old + 3, (newItem) => newItem % 17 === 0 ? 0 : 1),
})
...
export const generateMonkeys = () => ({
     ...
     */
    0: new Monkey(0, [89, 95, 92, 64, 87, 68], (old) => old * 11, (newItem) => newItem % 2 === 0 ? 7 : 4),
    1: new Monkey(1, [87, 67], (old) => old + 1, (newItem) => newItem % 13 === 0 ? 3 : 6),
    2: new Monkey(2, [95, 79, 92, 82, 60], (old) => old + 6, (newItem) => newItem % 3 === 0 ? 1 : 6),
    3: new Monkey(3, [67, 97, 56], (old) => old * old, (newItem) => newItem % 17 === 0 ? 7 : 0),
    4: new Monkey(4, [80, 68, 87, 94, 61, 59, 50, 68], (old) => old * 7, (newItem) => newItem % 19 === 0 ? 5 : 2),
    5: new Monkey(5, [73, 51, 76, 59], (old) => old + 8, (newItem) => newItem % 7 === 0 ? 2 : 1),
    6: new Monkey(6, [92], (old) => old + 5, (newItem) => newItem % 11 === 0 ? 3 : 0),
    7: new Monkey(7, [99, 76, 78, 76, 79, 90, 89], (old) => old + 7, (newItem) => newItem % 5 === 0 ? 4 : 5),
})

export const round = (monkeys: Record<number, Monkey>): Record<number, Monkey> => {
