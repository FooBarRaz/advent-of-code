...
export const getAllPointsOnLine = (line: Line): Point[] => {
  // check if line is horizontal or vertical
  if (line.a.x === line.b.x) {
    // vertical
    const minY = Math.min(line.a.y, line.b.y);
    const maxY = Math.max(line.a.y, line.b.y);
    return fillArray(maxY - minY + 1).map(
      (_, index) => new Point(line.a.x, minY + index)
    );
  } else if (line.a.y === line.b.y) {
    // horizontal
    const minX = Math.min(line.a.x, line.b.x);
    const maxX = Math.max(line.a.x, line.b.x);
    return fillArray(maxX - minX + 1).map(
      (_, index) => new Point(minX + index, line.a.y)
    );
  } else throw new Error("not a vertical or horizontal line");
};

export const initializeGrid = <T>(
  rows: number,
  columns: number,
  placeholder: T
): Grid<T> => {
...
export class Grid<T> {
  ...
  public get columns(): Array<Array<T>> {
    return fillArray(this.dimensions.columns).map((eachColumnIndex) => {
  ...
  allPoints(): Point[] {
    let ts = this.rows.reduce((prev, row, rowIndex) => {
      const points = row.map(
        (_, columnIndex) => new Point(columnIndex, rowIndex)
      );
      return [...prev, ...points];
    }, []);
    return ts as Point[];
  ...
  findCellByValue(val: T): Point {
    const point = this.allPoints().find(
      (point) => this.getPointValue(point) === val
    );
    if (!point) {
  ...
  getAdjacentPoints(
    point: Point,
    includeDiagonallyAdjacentCells = true
  ): PointAndValue<T>[] {
    let adjacentCells;
...
export function vertexWithShortestDistance(
  vertices: Point[],
...
export function getShortestPath(
  ...
  const unvisitedVertices = () =>
    allPoints.filter((point) => !sptSet.includes(point));
  while (unvisitedVertices().length) {
    const currentVertex = vertexWithShortestDistance(
      unvisitedVertices(),
      distances
    );
    ...
    neighbors.forEach((neighbor) => {
      ...
      if (hillClimber) {
        ...
        distanceToNeighbor =
          distances[currentVertexString] + points.getPointValue(neighbor.point);
      }
